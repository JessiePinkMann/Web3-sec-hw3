...
contract MultiSigWalletOptimizedTest is Test {
    MultiSigWalletOptimized walletOptimized;
    address[] owners;
    uint256 required;
    ...
    function setUp() public {
        ...
        walletOptimized = new MultiSigWalletOptimized(owners, required);
        payable(address(walletOptimized)).transfer(1 ether);
    }

    function testInitialization() public view {
        assertEq(walletOptimized.owners(0), owner1);
        assertEq(walletOptimized.owners(1), owner2);
        assertEq(walletOptimized.owners(2), owner3);
        assertEq(walletOptimized.required(), required);
    }

    function testSubmitTransaction() public {
        vm.startPrank(owner1);
        walletOptimized.submitTransaction(address(0x4), 100);
        assertEq(walletOptimized.getTransactionCount(), 1);
        ...
        (
            ...
            bool executed
        ) = walletOptimized.transactions(0);
    ...
    function testConfirmTransaction() public {
        ...
        walletOptimized.confirmTransaction(0);
        address[] memory confirmations = walletOptimized.getConfirmations(0);
        assertEq(confirmations.length, 1);
        assertEq(confirmations[0], owner1);

        (,,, uint256 confirmationCount,,) = walletOptimized.transactions(0);
    ...
    function testExecuteTransaction() public {
        ...
        (,,,,, bool executed) = walletOptimized.transactions(0);
        assertTrue(executed);
    }

    function testExecutionFailure() public {
        vm.startPrank(owner1);
        walletOptimized.submitTransaction(address(0x4), 100 ether);
    ...
    function testOnlyOwnerCanExecute() public {
        ...
        vm.prank(address(0x5));
        vm.expectRevert("Not owner");
        walletOptimized.executeTransaction(0);
    }
}
